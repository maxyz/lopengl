...
// Equilateral triangle matching the original geometry (side = 1, not circumradius-centred).
...
// Two-attribute vertex layout: position (location 0) + colour (location 1), interleaved.
...
int main(int argc, char *argv[]) {
    ...
    auto engine_result =
        create_engine("LOpenGL SDL3 - Attributes", WINDOW_WIDTH, WINDOW_HEIGHT, config);
    if (!engine_result) {
