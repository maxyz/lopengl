...
// Ambient and diffuse now come from the texture; only specular and shininess
// remain as flat material values pushed via uniform buffer.
struct material_uniforms_t {
    glm::vec4 specular_shininess; // .rgb = specular color, .w = shininess
};
...
constexpr material_uniforms_t MATERIAL = {
    .specular_shininess = {0.5f, 0.5f, 0.5f, 64.0f},
};
...
// Light pipeline reads only position (loc 0); normals and UVs are skipped.
// pos_normal_uv_buffer_descs provides the correct 32-byte stride so the GPU
// steps through the shared vertex buffer correctly.
...
void scene_t::render(SDL_GPUCommandBuffer *cmd, SDL_GPURenderPass *pass) {
    ...
    // Push VP and material/light once; model is pushed per draw call.
    ...
    // Light source cube: switch pipeline, re-push all vertex uniforms, push light
    // color for the strobe visualization. Over-push of specular/position is safe.
...
std::expected<scene_t, std::string> create_scene(engine_t &engine) {
    ...
    auto cube = create_pipeline(
        engine, {
                    ...
                    .fragment_shader          = "shaders/sdl3_15/cube_diffuse.frag.spv",
                    .vertex_uniform_buffers   = 3,
                    .fragment_uniform_buffers = 2, // slot 0: material, slot 1: light
                    .fragment_samplers        = 1, // slot 0: diffuse texture
    ...
    auto light_pipe = create_pipeline(
        engine, {
                    ...
                    .vertex_uniform_buffers   = 3,
                    .fragment_uniform_buffers = 1, // slot 0: light (ambient+diffuse for color)
                    .vertex_buffer_descs      = pos_normal_uv_buffer_descs,
...
int main(int argc, char *argv[]) {
    auto result = run_app(
        argc, argv, "SDL3 15 - Diffuse Map", WINDOW_WIDTH, WINDOW_HEIGHT, BACKGROUND_COLOR,
