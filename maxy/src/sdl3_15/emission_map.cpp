...
std::expected<scene_t, std::string> create_scene(engine_t &engine) {
    ...
    auto cube = create_pipeline(
        engine, {
                    ...
                    .fragment_shader          = "shaders/sdl3_15/cube_emission.frag.spv",
    ...
    auto mat = create_material(
        engine, {.texture_paths = {
                     ...
                     std::string(ASSETS_PATH) + "textures/matrix.jpg",
                 }}
    );
...
int main(int argc, char *argv[]) {
    auto result = run_app(
        argc, argv, "SDL3 15 - Emission Map", WINDOW_WIDTH, WINDOW_HEIGHT, BACKGROUND_COLOR,
