...
bool scene_t::update(input_t const &in) {
    ...
    // Mirrors the GLFW key_callback grave toggle from window_state.hpp.
    ...
    if (m_ui_mode) {
        // ColorEdit3 writes through the pointer every frame; no "on change" callback needed.
...
int main(int argc, char *argv[]) {
    auto result = run_app(
        argc, argv, "SDL3 15 - Light Controls", WINDOW_WIDTH, WINDOW_HEIGHT, BACKGROUND_COLOR,
