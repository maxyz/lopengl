...
// .x = shininess, .y = time (seconds) for the heartbeat pulse; .zw unused
...
void scene_t::render(SDL_GPUCommandBuffer *cmd, SDL_GPURenderPass *pass) {
    ...
    material_uniforms_t frame_material = {.shininess = 64.0f, .time = m_time};
...
std::expected<scene_t, std::string> create_scene(engine_t &engine) {
    ...
    auto cube = create_pipeline(
        engine, {
                    ...
                    .fragment_shader          = "shaders/sdl3_15/cube_emission_pulse.frag.spv",
                    .vertex_uniform_buffers   = 3,
                    .fragment_uniform_buffers = 2,
                    .fragment_samplers        = 3, // binding 0: diffuse, 1: specular, 2: emission
    ...
    auto mat = create_material(
        engine, {.texture_paths = {
                     ...
                     std::string(ASSETS_PATH) + "textures/matrix_full.jpg",
                 }}
    );
...
int main(int argc, char *argv[]) {
    auto result = run_app(
        argc, argv, "SDL3 15 - Emission Pulse", WINDOW_WIDTH, WINDOW_HEIGHT, BACKGROUND_COLOR,
