...
struct positions_t {
    // view_pos removed: in camera-relative world space the camera is at the origin.
    glm::vec4 light_pos;
};
...
int main(int argc, char *argv[]) {
    ...
    auto engine_result = create_engine(
        "SDL3 13 - Ex 2 Camrel: Camera-Relative World Space", WINDOW_WIDTH, WINDOW_HEIGHT, config
    );
    if (!engine_result) {
    ...
    while (true) {
        ...
        auto frame = render_frame(
            ...
            [&](SDL_GPUCommandBuffer *cmd_buf, SDL_GPURenderPass *pass) {
                glm::mat4 full_view = camera.view_matrix();
                // Strip the translation from the view matrix. Object positions are
                // already camera-relative, so only the rotation part is needed here.
                glm::mat4 view = glm::mat4(glm::mat3(full_view));
                glm::mat4 projection =
                    glm::perspective(glm::radians(camera.fov), aspect_ratio(engine), 0.1f, 100.0f);

                // Subtract camera position so all positions sent to the GPU are
                // relative to the camera. In a large world this subtraction would be
                // done in double precision on the CPU to preserve accuracy.
                positions_t positions{
                    .light_pos = glm::vec4(light_rot - camera.position, 0.0f),
                };
                ...
                for (Uint32 i = 0; i < example_cube_positions.size(); ++i) {
                    ...
                    // Translate to camera-relative position before rotating.
                    glm::vec3 camrel_pos = example_cube_positions[i] - camera.position;
                    glm::mat4 model      = glm::rotate(
                        glm::translate(glm::mat4(1.0f), camrel_pos), glm::radians(angle),
                ...
                glm::mat4 light_model = glm::scale(
                    glm::translate(glm::mat4(1.0f), light_rot - camera.position), glm::vec3(0.2f)
                );
