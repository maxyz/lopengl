...
// Lighting parameters sent to the vertex stage (light_color, light_strengths).
...
int main(int argc, char *argv[]) {
    ...
    auto engine_result =
        create_engine("SDL3 13 - Ex 3: Gouraud Shading", WINDOW_WIDTH, WINDOW_HEIGHT, config);
    if (!engine_result) {
    ...
    // 5 vertex uniforms: model(0), view(1), projection(2), light_pos(3), vertex_lighting(4).
    // 1 fragment uniform: fragment_lighting(0) -- only object_color in the fragment stage.
    auto cube_pipeline_result = create_pipeline(
        engine, {
                    .vertex_shader            = "shaders/sdl3_13/cube_gouraud.vert.spv",
                    .fragment_shader          = "shaders/sdl3_13/cube_gouraud.frag.spv",
                    .vertex_uniform_buffers   = 5,
    ...
    vertex_lighting_t vertex_lighting = {
        .light_color     = {1.0f, 1.0f, 1.0f, 0.0f},
        .light_strengths = {0.1f, 1.0f, 0.5f, 32.0f},
    };
    fragment_lighting_t fragment_lighting = {
    ...
    while (true) {
        ...
        auto &ls = vertex_lighting.light_strengths;
        ...
        auto frame = render_frame(
            ...
            [&](SDL_GPUCommandBuffer *cmd_buf, SDL_GPURenderPass *pass) {
                ...
                push_vertex_uniform(cmd_buf, 4, vertex_lighting);
                push_fragment_uniform(cmd_buf, 0, fragment_lighting);
