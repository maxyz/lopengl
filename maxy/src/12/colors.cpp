...
std::expected<hooks_t, std::string> init_shaders() {
    auto shader = Shader::build("shaders/12_colors.vert", "shaders/12_colors.frag");
    if (!shader) {
        return std::unexpected(shader.error());
    }
    auto light_shader = Shader::build("shaders/12_light.vert", "shaders/12_light.frag");
    if (!light_shader) {
    ...
    shader->set_vec3("objectColor", glm::vec3(1.f, .5f, .31f));
    shader->set_vec3("lightColor", glm::vec3(1.f, 1.f, 1.f));
