...
// Modulates diffuse and ambient by sin(time). sin() ranges [-1,1] so the
// multipliers produce negative values on the down-half of the cycle; the GPU
// clamps these to black, creating the strobe off-phase without an explicit
// toggle. Ambient is kept proportional to diffuse so the color ratio is
// preserved throughout the cycle.
...
int main(int argc, char *argv[]) {
    auto result = run_app(
        argc, argv, "SDL3 14 - Strobe", WINDOW_WIDTH, WINDOW_HEIGHT, BACKGROUND_COLOR, create_scene
    );
    if (!result) {
