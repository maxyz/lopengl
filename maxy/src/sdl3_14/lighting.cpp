...
//   normals, and light position to view space so the fragment shader can use
//   view_dir = normalize(-frag_pos) because the camera is at the origin there.
//   Camera-relative world space achieves the same property on the CPU side
//   (subtract camera.position before uploading). The fragment shader is
//   identical; the normal matrix simplifies from mat3(transpose(inverse(view*model)))
//   to mat3(transpose(inverse(model))) because view is rotation-only.
//   Both share the same shaders/sdl3_14/cube.{vert,frag}.spv.
...
void scene_t::render(SDL_GPUCommandBuffer *cmd, SDL_GPURenderPass *pass) {
    ...
    light_uniforms_t frame_light = light;
...
std::expected<scene_t, std::string> create_scene(engine_t &engine) {
    ...
    auto light_pipe = create_pipeline(
        engine, {
                    ...
                    .fragment_shader        = "shaders/sdl3_14/light.frag.spv",
                    .vertex_uniform_buffers = 3,
                    .vertex_buffer_descs    = pos_normal_uv_buffer_descs,
...
int main(int argc, char *argv[]) {
    auto result = run_app(
        argc, argv, "SDL3 14 - Lighting (light_t)", WINDOW_WIDTH, WINDOW_HEIGHT, BACKGROUND_COLOR,
        create_scene
    );
    if (!result) {
