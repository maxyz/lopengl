...
// Per-component material colors model how each surface reflects ambient, diffuse,
// and specular light independently. Using vec4 (not vec3) avoids std140 padding
// issues: vec3 in std140 is aligned to 16 bytes but glm::vec3 is 12 on the CPU.
...
// Standard white light with different per-component intensities.
...
int main(int argc, char *argv[]) {
    auto result = run_app(
        argc, argv, "SDL3 14 - Materials", WINDOW_WIDTH, WINDOW_HEIGHT, BACKGROUND_COLOR,
        create_scene
    );
    if (!result) {
