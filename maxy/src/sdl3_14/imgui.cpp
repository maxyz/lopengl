...
constexpr std::array<std::pair<const char *, material_uniforms_t>, 23> materials = {{
    {"emerald",
...
bool scene_t::update(input_t const &in) {
    ...
    ImGui::LabelText("Material", "%s", materials[m_current_mat].first);
    ImGui::LabelText("Textures", "%s", m_textures_on ? "on" : "off");
    ImGui::PopItemWidth();
    ImGui::End();
...
void scene_t::render(SDL_GPUCommandBuffer *cmd, SDL_GPURenderPass *pass) {
    ...
    push_fragment_uniform(cmd, 0, materials[m_current_mat].second);
...
int main(int argc, char *argv[]) {
    auto engine = create_engine(
        "SDL3 14 - ImGui Overlay", WINDOW_WIDTH, WINDOW_HEIGHT, parse_engine_args(argc, argv)
    );
    if (!engine) {
