// Each cube has its own rotation axis, speed, and starting phase,
// making the rotations fully independent and visually chaotic.
#include <algorithm>
#include <array>
#include <print>
#include <string>
...
// Unique rotation speed per cube (degrees per second).
constexpr std::array<float, 10> rotation_speeds = {{
    25.0f,
    43.0f,
    17.0f,
    38.0f,
...
// Starting angle offset per cube (degrees).
constexpr std::array<float, 10> rotation_phases = {{
    0.0f,
    36.0f,
    72.0f,
    108.0f,
...
int main(int argc, char *argv[]) {
    ...
    auto engine_result = create_engine("SDL3 - Chaotic Cubes", WINDOW_WIDTH, WINDOW_HEIGHT, config);
    if (!engine_result) {
