...
constexpr float LEVEL_STEP    = 0.5f; // units per second
    ...
    while (poll_events()) {
        ...
        auto frame = render_frame(
            engine, background_color, [&](SDL_GPUCommandBuffer *cmd_buf, SDL_GPURenderPass *pass) {
                // Push the level before draw() -- SDL3_GPU applies the most
                // recently pushed uniform data to the next draw call.
