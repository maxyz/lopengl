...
int main(int argc, char *argv[]) {
    ...
    auto engine_result =
        create_engine("SDL3 ex 7.8.2 - Wrapping Modes", WINDOW_WIDTH, WINDOW_HEIGHT, config);
    if (!engine_result) {
    ...
    auto mesh_result = create_textured_mesh(
        engine, {
                    ...
                    .address_modes = {
                        SDL_GPU_SAMPLERADDRESSMODE_CLAMP_TO_EDGE, // container: edge pixels stretch
                        SDL_GPU_SAMPLERADDRESSMODE_REPEAT,        // face: tiles when coord > 1
                    },
                }
    );
    if (!mesh_result) {
