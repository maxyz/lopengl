...
std::expected<std::vector<std::function<void()>>, std::string> init_shaders() {
  auto res = Shader::build("shaders/ex_8.19.2.vert", "shaders/ex_8.19.2.frag");
  if (!res.has_value()) {
  ...
  auto g = [p, vao, texture, texture1]() {
    ...
    glm::mat4 transform = glm::mat4(1.f);
    transform = glm::translate(transform, glm::vec3(-.5f, .5f, .0f));
    auto scale = static_cast<float>(std::sin(glfwGetTime()));
    transform = glm::scale(transform, scale * glm::vec3(1.f));
    auto loc = glGetUniformLocation(p, "transform");
    glUniformMatrix4fv(loc, 1, GL_FALSE, glm::value_ptr(transform));
  ...
  std::vector<std::function<void()>> v{f, g};
  return v;
}
