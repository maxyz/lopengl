...
std::pair<vaos_t, vbos_t> load_buffers() {
  ...
  glBufferData(
      GL_ARRAY_BUFFER, sizeof(pyramid_vertices), pyramid_vertices, GL_STATIC_DRAW
  );
...
void SceneRenderer::render_scene_set_lighting() {
  ...
  for (size_t index = 0; index < state.spot_lights.size(); ++index)
    set_spot_light(
        program, std::format("spot_lights[{}]", index), state.spot_lights[index]
    );

  const flashlight_t active_flashlight = state.flashlight_on
      ? preset.flashlight
