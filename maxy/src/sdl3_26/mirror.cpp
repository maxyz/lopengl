...
// The SCREEN_QUAD spans NDC (-1,-1)-(1,1); after this transform the overlay occupies
// x:[-0.3, 0.3], y:[0.5, 1.1] — the upper-center area.
...
struct scene_t {
    ...
    gpu_pipeline_t overlay_pipeline; // PiP quad with transform
    ...
    void draw_world(
        ...
        glm::mat4 const &proj, glm::vec3 const &cam_front
    );
    ...
    void render_overlay(SDL_GPUCommandBuffer *cmd, SDL_GPURenderPass *pass);
};
...
std::expected<scene_t, std::string> create_scene(engine_t &engine) {
    ...
    // Overlay pipeline: renders the PiP quad in pass3 (no depth attachment).
    auto overlay_pipe = create_pipeline(
        engine, {
    ...
    if (!overlay_pipe) return std::unexpected(overlay_pipe.error());
    scene.overlay_pipeline = std::move(*overlay_pipe);
...
void scene_t::draw_world(
    ...
    flashlight_uniforms_t flashlight_uniform = {
        .direction    = glm::vec4(cam_front, 0.0f),
...
void scene_t::render_mirror(SDL_GPUCommandBuffer *cmd, SDL_GPURenderPass *pass) {
    // Mirror camera: same position, yaw reversed by 180 degrees, pitch negated.
    // This simulates a rearview mirror -- looking backwards and upward.
    ...
    draw_world(cmd, pass, view, proj, front);
}

void scene_t::render_main(SDL_GPUCommandBuffer *cmd, SDL_GPURenderPass *pass) {
    glm::mat4 view = camera.rotation_view();
    glm::mat4 proj = glm::perspective(glm::radians(camera.fov), m_aspect_ratio, 0.1f, 100.0f);
    draw_world(cmd, pass, view, proj, camera.front());
}

void scene_t::render_overlay(SDL_GPUCommandBuffer *cmd, SDL_GPURenderPass *pass) {
    constexpr uint32_t no_effects = 0u;

    SDL_BindGPUGraphicsPipeline(pass, overlay_pipeline.get());
    ...
    push_fragment_uniform(cmd, 0, no_effects);
...
int main(int argc, char *argv[]) {
    auto engine = create_engine(
        "SDL3 26 - Mirror", WINDOW_WIDTH, WINDOW_HEIGHT, parse_engine_args(argc, argv)
    );
    if (!engine) {
    ...
    std::array<pass_desc_t, 3> passes = {{
        ...
        {.load_op = SDL_GPU_LOADOP_LOAD, .draw = [&](auto cmd, auto pass) {
             scene->render_overlay(cmd, pass);
