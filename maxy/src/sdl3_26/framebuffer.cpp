...
// Fullscreen quad for the screen pass. NDC y=-1 is top in Vulkan/SDL3 GPU, UV v=0 is top.
struct screen_vertex_t {
    glm::vec2 position;
    glm::vec2 tex_coord;
};

// SDL3 GPU uses OpenGL-style NDC: y=+1 is top, y=-1 is bottom. The off-screen texture stores
// rows top-to-bottom (Vulkan convention), so NDC top (y=+1) must map to UV v=0.
...
struct scene_t {
    ...
    uint32_t   m_kernel_idx    = 0; // 0=none, 1=blur, 2=edge, 3=sharpen, 4=emboss
...
std::expected<scene_t, std::string> create_scene(engine_t &engine) {
    ...
    // Screen (post-processing) pipeline renders a fullscreen quad to the swapchain.
    // No depth test: the quad covers everything and there is no depth attachment in pass 2.
    ...
    // Screen sampler: clamp to edge so convolution kernels don't bleed over the screen border.
...
int main(int argc, char *argv[]) {
    auto engine = create_engine(
        "SDL3 26 - Framebuffer", WINDOW_WIDTH, WINDOW_HEIGHT, parse_engine_args(argc, argv)
    );
    if (!engine) {
