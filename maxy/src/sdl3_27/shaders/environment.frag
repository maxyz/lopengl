...
layout(set = 2, binding = 2) uniform samplerCube skybox;
...
void main() {
    ...
    // Environment reflection: frag_pos is world-space relative to camera,
    // so normalize(frag_pos) is a world-space incident direction suitable
    // for sampling the skybox cubemap.
    vec3 incident = normalize(frag_pos);
    vec3 reflect_dir = reflect(incident, norm);
    vec3 env_color = texture(skybox, reflect_dir).rgb;
    frag_color = vec4((result + env_color) / 2.0, diffuse_texel.a);
}
