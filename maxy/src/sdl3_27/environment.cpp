...
void scene_t::render_scene(SDL_GPUCommandBuffer *cmd, SDL_GPURenderPass *pass) {
    ...
    // The environment cubemap is bound at fragment sampler slot 2 for all
    // lit pipelines; slots 0 and 1 are filled per-draw by the material.
    SDL_GPUTextureSamplerBinding env_binding = {cubemap_texture.get(), cubemap_sampler.get()};
...
int main(int argc, char *argv[]) {
    auto engine = create_engine(
        "SDL3 27 - Environment Mapping", WINDOW_WIDTH, WINDOW_HEIGHT, parse_engine_args(argc, argv)
    );
    if (!engine) {
