...
std::expected<cbs_t, std::string> init_shaders() {
    auto res = Shader::build("shaders/10_camera_circle.vert", "shaders/10_camera_circle.frag");
    if (!res.has_value()) {
    ...
    auto f = [p, vao, texture, texture1](uint64_t e, float delta) {
        ...
        if (e & event_t::camera_left) {
            camera_pos -= delta * CAM_SPEED * glm::normalize(glm::cross(camera_front, UP));
        }
        ...
        for (unsigned int i = 0; i < 10; ++i) {
            angle = 20.f * i;
            if (i % 3 == 0) angle = glfwGetTime() * 25.f;
...
void scroll_callback(GLFWwindow *window, double x_offset, double y_offset) {
    update_fov(static_cast<float>(y_offset));
}
