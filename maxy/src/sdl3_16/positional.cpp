...
void scene_t::render(SDL_GPUCommandBuffer *cmd, SDL_GPURenderPass *pass) {
    ...
    push_vertex_uniform(cmd, 1, view);
    push_vertex_uniform(cmd, 2, projection);
    push_fragment_uniform(
        cmd, 0, frame_light
    ); // light.frag reads PositionalLightBlock at binding 0
...
std::expected<scene_t, std::string> create_scene(engine_t &engine) {
    ...
    auto cube = create_pipeline(
        engine, {
                    ...
                    .fragment_shader          = "shaders/sdl3_16/positional.frag.spv",
    ...
    auto light_pipe = create_pipeline(
        engine, {
                    ...
                    .fragment_shader          = "shaders/sdl3_16/light.frag.spv",
                    .vertex_uniform_buffers   = 3,
...
int main(int argc, char *argv[]) {
    auto result = run_app(
        argc, argv, "SDL3 16 - Positional Light", WINDOW_WIDTH, WINDOW_HEIGHT, BACKGROUND_COLOR,
