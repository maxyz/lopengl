...
std::expected<std::string, std::string> get_asset_path(const std::string &filename) {
#ifdef ASSETS_PATH
...
std::expected<Image, std::string> load_image(const std::string &filename) {
    ...
    Image image;
    auto *data =
        stbi_load(asset_path->c_str(), &image.width, &image.height, &image.channel_count, 0);
    if (!data) return std::unexpected(std::format("Failed to load texture ({})", filename));
...
std::expected<GLenum, std::string> guess_format(const Image &image) {
    if (image.channel_count == 1) return GL_RED;
    if (image.channel_count == 3) return GL_RGB;
    if (image.channel_count == 4) return GL_RGBA;
    return std::unexpected(
        std::format(
            "Could not detect image format, invalid amount of channels {}", image.channel_count
        )
    );
}
