...
std::expected<hooks_t, std::string> init_shaders(GLFWwindow *window) {
  ...
  auto shader = Shader::build("shaders/15_view_emission.vert",
                              "shaders/15_view_emission.frag");
  if (!shader) {
  ...
  auto load_texture_emission_res = load_texture("textures/matrix.jpg");
  if (!load_texture_emission_res) {
    return std::unexpected(load_texture_emission_res.error());
  }
  textures_t ts = {.diffuse = *load_texture_res,
                   .specular = *load_texture_specular_res,
                   .emission = *load_texture_emission_res};
  ...
  auto f = [ps, vs, ts](input_t input, float delta) {
    ...
    glBindTexture(GL_TEXTURE_2D, ts.emission);
    emission_map_t specular_map = {
        .diffuse = 0,
        .specular = 1,
        .emission = 2,
        .shininess = 64.f,
    ...
    set_vec3(ps.view, "light_pos", state.light.position);
    set_vec3(ps.view, "view_pos", state.camera.position);
    set_light(ps.view, "light", state.light);
    set_emission_map(ps.view, "emission_map", specular_map);
    ...
    set_light(ps.light, "light", state.light);
