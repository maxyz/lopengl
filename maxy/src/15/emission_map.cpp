...
std::expected<SceneRenderer, std::string> SceneRenderer::create(GLFWwindow *window) {
    auto shader = Shader::build("shaders/15_view_emission.vert", "shaders/15_view_emission.frag");
    if (!shader) {
...
void SceneRenderer::render_scene() {
    ...
    glBindTexture(GL_TEXTURE_2D, m_textures.emission);
    ...
    set_emission_map(m_programs.view.program_id(), "emission_map", specular_map);
    ...
    set_light(m_programs.light.program_id(), "light", state.light);
