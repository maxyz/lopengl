...
std::expected<unsigned int, std::string> compile_shader(const GLenum type, const char *source);
std::expected<unsigned int, std::string> link_shaders(std::vector<unsigned int> shaders);
unsigned int buffers();

std::expected<std::vector<cb_t>, std::string> init_shaders() {
    auto res = Shader::build("shaders/ex_6.8.2.vert", "shaders/ex_6.8.2.frag");
    if (!res.has_value()) {
