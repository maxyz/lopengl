...
void scene_t::render(SDL_GPUCommandBuffer *cmd, SDL_GPURenderPass *pass) {
    ...
    // Pipeline selection is the only thing that changes between depth modes.
...
std::expected<scene_t, std::string> create_scene(engine_t &engine) {
    ...
    // One pipeline per depth mode — compare op is pipeline state in SDL3 GPU.
    for (size_t i = 0; i < DEPTH_MODES.size(); ++i) {
        auto pipe = create_pipeline(
            engine, {
                        .vertex_shader            = "shaders/sdl3_22/depth.vert.spv",
                        .fragment_shader          = "shaders/sdl3_22/depth.frag.spv",
...
int main(int argc, char *argv[]) {
    auto result = run_app(
        argc, argv, "SDL3 22 - Depth Testing", WINDOW_WIDTH, WINDOW_HEIGHT, BACKGROUND_COLOR,
