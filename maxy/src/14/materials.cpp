...
std::expected<hooks_t, std::string> init_shaders() {
    auto shader = Shader::build("shaders/14_view.vert", "shaders/14_view.frag");
    if (!shader) {
        return std::unexpected(shader.error());
    }
    auto light_shader = Shader::build("shaders/14_light.vert", "shaders/14_light.frag");
    if (!light_shader) {
    ...
    auto f = [p, light_id, cube_vao, texture, texture1, material](uint64_t e, float delta) {
        ...
        set_vec3(p, "light_pos", light_position);
