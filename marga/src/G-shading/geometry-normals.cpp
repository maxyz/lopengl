...
void SceneRenderer::createShaders()
{
    ...
    this->normalShader = new Shader("shaders/model-normal-vertex.glsl", "shaders/model-normal-frag.glsl", "shaders/model-normal-geom.glsl");
}
...
void SceneRenderer::init()
{
    ...
    this->sceneShader->setFloat("time", -3.14159/2.0);
}
...
void SceneRenderer::renderScene(SceneState &state) 
{
    ...
    this->normalShader->setMatrix4fv("view", glm::value_ptr(view));
    this->normalShader->setMatrix4fv("projection", glm::value_ptr(projection));
    this->normalShader->setMatrix4fv("model", glm::value_ptr(model));
    this->backpack->Draw(*(this->normalShader));
