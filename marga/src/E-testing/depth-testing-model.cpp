...
Camera camera = Camera(glm::vec3(-16.0f, 5.0f, -2.0f));
...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(INITIAL_WIDTH, INITIAL_HEIGHT, "Depth Testing", NULL, NULL);
    if (window == NULL)
    {
    ...
    // Read shaders
    Shader multiLightShader("shaders/vertex.glsl", "shaders/multi-fragment.glsl");
    Shader depthBufferShader("shaders/vertex.glsl", "shaders/depth-buffer.glsl");
    Shader foggyShader("shaders/vertex.glsl", "shaders/foggy.glsl");
    ...
    float planeVertices[] = {
        // positions          // texture Coords (note we set these higher than 1 (together with GL_REPEAT as texture wrapping mode). this will cause the floor texture to repeat)
         25.0f, 0.1f,  25.0f,  0.0f, 1.0f, 0.0f, 2.0f, 0.0f,
        -25.0f, 0.1f,  25.0f,  0.0f, 1.0f, 0.0f, 0.0f, 0.0f,
        -25.0f, 0.1f, -25.0f,  0.0f, 1.0f, 0.0f, 0.0f, 2.0f,

         25.0f, 0.1f,  25.0f,  0.0f, 1.0f, 0.0f, 2.0f, 0.0f,
        -25.0f, 0.1f, -25.0f,  0.0f, 1.0f, 0.0f, 0.0f, 2.0f,
         25.0f, 0.1f, -25.0f,  0.0f, 1.0f, 0.0f, 2.0f, 2.0f								
    };    
    // plane VAO
    ...
    Texture marble = Texture("marble.jpg", "../media");
    Texture metal = Texture("metal.png", "../media");
    multiLightShader.setInt("material.texture_diffuse1", 0);
    foggyShader.setInt("material.texture_diffuse1", 0);

    stbi_set_flip_vertically_on_load(true);
    Model cottage = Model("models/cottage_obj.obj");
    ...
    PositionalLight positionalLights[] = {
    	PositionalLight(glm::vec3(10.0f,  8.2f, -1.0f), glm::vec3( 1.0f,  1.0f,  1.0f), 0.2f, 0.5f),
	    PositionalLight(glm::vec3(-4.3f,  8.3f, -1.0f), glm::vec3( 0.0f,  0.0f,  1.0f), 0.2f, 0.5f),
    	PositionalLight(glm::vec3(-8.0f,  7.0f, -6.0f), glm::vec3( 1.0f,  0.0f,  0.0f), 0.2f, 0.5f),
    	PositionalLight(glm::vec3(-3.0f,  6.0f,  3.0f), glm::vec3( 0.0f,  1.0f,  0.0f), 0.2f, 0.5f)
    };  
    ...
    Shader ourShader = multiLightShader;
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        if (glfwGetInputMode(window, GLFW_CURSOR) != GLFW_CURSOR_DISABLED) {
            ...
            if (ImGui::Button("Normal Shader"))
                ourShader = multiLightShader;
            ImGui::SameLine();
            if (ImGui::Button("Depth Buffer Shader"))
                ourShader = depthBufferShader;
            ImGui::SameLine();
            if (ImGui::Button("Foggy Shader"))
                ourShader = foggyShader;
        }
        ImGui::SeparatorText("");
        ...
        // ** Projection **
        glm::mat4 projection;
        projection = glm::perspective(glm::radians(camera.Zoom), width/height, 0.1f, 200.0f);
