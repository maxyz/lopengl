...
void main()
{
  ...
  FragColor = vec4(outputColor, texture(material.texture_diffuse1, TexCoords)[3]);
}  
...
vec3 CalcDirLight(DirLight light, vec3 normal, vec3 viewDir)
{
    ...
    // specular shading
    float spec = CalcSpec(lightDir, viewDir, normal);
    // combine results
...
vec3 CalcSpotLight(SpotLight light, vec3 norm, vec3 fragPos, vec3 viewDir)
{
    ...
    float spec = CalcSpec(lightDir, viewDir, norm);
