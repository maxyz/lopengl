...
int main()
{
    ...
    // Read shaders
    Shader ourShader("shaders/vertex.glsl", "shaders/attenuation-fragment.glsl");
    ...
    // Starting lighting values (position, color, ambient, diffuse, specular, constant, linear, quadratic)
    Light light = { {1.2f, 1.0f, 2.0f}, glm::vec3( 1.0f,  1.0f,  1.0f), 0.2f, 0.5f, 1.0f, 1.0f, 0.09f, 0.032f, };
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        if (glfwGetInputMode(window, GLFW_CURSOR) != GLFW_CURSOR_DISABLED) {
            ...
            ImGui::Checkbox("Rotate light", &light_moves);
        }
        ImGui::SeparatorText("");
        ...
        if (light_moves) {
            light.position.x = (float) 1.2f*sin(glfwGetTime());
            light.position.y = (float) 1.0f*cos(glfwGetTime());
        }
        ourShader.setVec3f("light.position", glm::value_ptr(light.position));
