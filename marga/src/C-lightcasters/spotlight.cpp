...
int main()
{
    ...
    // Read shaders
    Shader ourShader("shaders/vertex.glsl", "shaders/spotlight-fragment.glsl");
    ...
    Light light = { {1.2f, 1.0f, 2.0f}, glm::vec3( 1.0f,  1.0f,  1.0f), 0.5f, 0.9f, 1.0f, 1.0f, 0.09f, 0.032f, 12.5f, 15.0f};
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        if (glfwGetInputMode(window, GLFW_CURSOR) != GLFW_CURSOR_DISABLED) {
            ...
            ImGui::SliderFloat("Attenuation Constant", &light.constant, 0.0f, 1.0f);
            ImGui::SliderFloat("Attenuation Linear", &light.linear, 0.0f, 1.0f);
            ImGui::SliderFloat("Attenuation Quadratic", &light.quadratic, 0.0f, 1.0f);
            ImGui::InputFloat("Cut Off angle (in degrees)", &light.cutOff, 1.0f, 10.0f);
            ImGui::InputFloat("Outer Cut Off angle (in degrees)", &light.outerCutOff, 1.0f, 10.0f);
        }
        ImGui::SeparatorText("");
        ...
        ourShader.setFloat("light.cutOff", glm::cos(glm::radians(light.cutOff)));
        ourShader.setFloat("light.outerCutOff", glm::cos(glm::radians(light.outerCutOff)));
