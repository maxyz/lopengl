...
bool pPreviouslyPressed = false;
bool perspectiveLock = false;
...
void processInput(GLFWwindow *window, Camera &cam)
{
    ...
    if(!pPreviouslyPressed && glfwGetKey(window, GLFW_KEY_P) == GLFW_PRESS) {
        perspectiveLock = !perspectiveLock;
        std::cout << "DEBUG << PERSPECTIVE_LOCK SET TO: " << perspectiveLock << "\n";
    }
    pPreviouslyPressed = glfwGetKey(window, GLFW_KEY_P) == GLFW_PRESS;
