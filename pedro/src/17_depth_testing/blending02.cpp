...
int main()
{
    ...
    Texture2D windowTexture("../media/blending_transparent_window.png", PNG);
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        // windows
        glBindVertexArray(transparentVAO);
        windowTexture.activate();

        std::map<float, glm::vec3> sorted_windows;
        for (size_t i = 0; i < windows.size(); i++)
        {
            float distance = glm::length(cam.position - windows[i]);
            sorted_windows[distance] = windows[i];
        }        

        for (std::map<float, glm::vec3>::reverse_iterator it = sorted_windows.rbegin(); it != sorted_windows.rend(); it++)
        {
            model = glm::mat4(1.0f);
